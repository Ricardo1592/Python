...
def exibir_imagens():
    ...
    frame_left = ctk.CTkFrame(janela, width=500, height=800)
    frame_left.pack(side='left', padx=40, pady=50)
    # Cria o canvas e o scrollbar vertical
    canvas = ctk.CTkCanvas(janela, width=2230, height=1500)
    ...
    for i, arquivo in enumerate(arquivos):
        ...
        var = IntVar()
        checkbox = ctk.CTkCheckBox(frame_canvas, variable=var, command=lambda i=i: checkbox_clicado(i))
        checkbox.var = var

        # Cria um Label para exibir a imagem
        label = Label(frame_canvas, image=imagem)
        label.image = imagem
    ...
    botao_teste = Button(frame_canvas, text="Imprimir Checkboxes Clicados", command=imprimir_checkboxes_clicados, width=130)
